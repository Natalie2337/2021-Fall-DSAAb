...
public class Particle {
    ...
    private static final double G = 6.67259e-11;
    private Cell cell;
...
//        this.ay = -0.0005;
     ...
     */
    public double timeToHitVerticalWall(int size) {
        if      (vx > 0) return (size - rx - radius) / vx;
     ...
     */
    public double timeToHitHorizontalWall(int size) {
        if      (vy > 0) return (size - ry - radius) / vy;
...
//        double dxy = Math.sqrt(Math.pow(dx,2)+Math.pow(dy,2));
    ...
    public void acceleration(Particle that) {
        if(this == that) return;
        double d = dis(that);
        double force = G * that.mass * this.mass / (d * d);
    ...
    public void acceleration(Cell cell) {
        double d = dis(cell);
        if (d == 0) return;
        double force = G * cell.getMass() * this.mass / (d * d);
