...
//        pq.insert(e);
    ...
    public void simulate0(double limit) {
        pq = new MinPQ<>();
        for (Particle value : particles) {
            predict(value,1.0 / HZ / n);
        }
...
//            if (!e.isValid()) continue;
    ...
    public void printInformation0(double[] times, int[] index) {
        pq = new MinPQ<>();
        for (Particle value : particles) {
            predict(value,1.0 / HZ / n);
        }
...
//            }
    ...
    private void  update() {
        ...
        updateAccBH();
        accelerate(1.0 / n);
        for (Particle particle : particles) predict(particle,1.0 / n);
