...
public class BSTree<Key extends Comparable<Key>, Value> implements Iterable<KVPair<Key,Value>> {
    ...
    public KVPair<Key,Value> min() {
        TreeNode<Key,Value> node = min(root);
        return (node==null) ? null : new KVPair<Key,Value>(node.key, node.value);
    }

    private TreeNode<Key, Value> next( TreeNode<Key, Value> node ) {
        if( node == null )
            return null;
        if( node.right != null )
    ...
    private TreeNode<Key,Value> delMin( TreeNode<Key,Value> node ) {
        if( node == null )
            return null;
        while( node.left != null )
            node = node.left;
        ...
        while( node != null ) {
            ...
            if( compare < 0 )
                ...
                if( node.left == null ) {
                    ...
                    TreeNode<Key,Value> minNode = delMin(node.right);
                    Value ret = node.value;
